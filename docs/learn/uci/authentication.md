...
A desired property of threshold signature schemes is _robustness_ in the sense that the protocol can tolerate the cheating of a limited number of participants. Ideally, a robust scheme will run successfully despite cheating participants. If this goal is attained after re-running the protocol, we refer to the property as _semi-robustness_.
...
ii) allowing a blockchain network to distribute a **static long-running verification key** with respect to which different sets of signers can produce signatures. This allows the verification key associated with each subnet to stay static while the set of potential signers can vary easily. This is a key feature for blockchains, i.e. dynamic networks whose participating nodes arbitrarily join and leave.
...
-  Providing _optional_ semi-robustness for the signing protocol. Each subnet can trade of size of the signers' set for semi-robustness guarantee. The more signers means the heavier protocol but an increased guarantee of successfully generating a valid signature.
