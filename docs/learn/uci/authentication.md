...
To prevent spamming, false identity and data tampering, Topos XSP leverages _digital signatures_ to ensure Certificates are attributed to the right originators, i.e. the subnets that emit them to prove and ensure that integrity is retained during their propagation for the purposes of cross-subnet transactions. As seen previously, signatures are included with Certificates and hence can be validated prior to verifying the Certificate validity using a public key of the associated/underlying subnet.
