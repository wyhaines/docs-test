...
ICE-FROST is used in XSP for signing certificates by the sidechain validators. The main purpose of signing is authenticating the sidechain creating the certificate and verifying the integrity of the certificate i.e., ensuring that the certificate was not altered while in transit. It is assumed that sidechains are running a BFT consensus mechanism, so for any randomly chosen set of signers, one-third of the participants in the set are assumed to be malicious. Hence, the XSP requires that at least two-thirds of the validators in a randomly chosen set (the threshold) sign the certificate. This ensures that, unless an adversary controls a supermajority of the validators, it is not possible for a sidechain to commit to a state which is unknown to honest validators in the certificate.

When a sidechain registration takes place, the initial set of validators runs the initial DKG phase, as a result of which they obtain a static ICE-FROST public key that is required to verify certificate signatures. This public key is included in the registration and stays static for the lifetime of the given sidechain. The set of validators is updated every *epoch* (e.g., approximately every six hours in our case). This is when an update of the shares phase takes place. Since the public key is designed to be static, the redistribution of shares does not change the group signing and verification keys. Note that both the initial and redistribution phases are *dealerless*.

The certificates are signed and then propagated with the signature by the sidechain's XSP node. When it reaches the mainchain and/or other XSP nodes, the signature is verified using the sidechain's public key. The signature is of the same format as a regular Schnorr signature (even though it is generated by a group of signers instead of individual ones). Checking the certificate signature before processing the certificate effectively prevents spamming, in the case if an adversary sends multiple certificates with invalid signatures, since it is almost a free operation.
...
Signing protocol of ICE-FROST needs a signature aggregator (SA). Any honest participant is capable of perform a role of a SA that can be chosen randomly for signing each certificate. SA is then in charge of choosing a group of signers with at least *t* members to generate the *(t,n)* threshold signature. However, generation of a valid signature requires cooperation of at least *t* **honest** participants. According to the 1/3 Byzantine assumption over the blockchain environment, SA may choose larger set of signers to ensure at least *t* honest participants are among the chosen ones with high probability. There is a trade-off between the size of the set and success probability of generating of a valid signature. The failure rate of zero-percent can be achieved by choosing at least one-third plus t of the participants and sets of smaller sizes will result higher failure percentages. 

After choosing the set of signers, SA will fetch each chosen participant's commitment that has been published and stored during the preprocessing round and sends the message, containing the set of signers and their commitments, to all chosen signers. Each participant checks the validity of the received message and calculates a _binding value_ to the message and the group of signers. Then, each one of them derives another commitment value that binds the message, the set of signing participants, and each participant’s commitment to each signature share. This latter commitment will be used to calculate the group's commitment value. The challenge value of the Schnorr's signature will then be generated by applying a hash function on the group's commitment value, public key and the message. Each participant responds to challenge, using its committed nonces (from preprocessing), and the long-term signing key share. SA checks the validity of responses and if passed adds all the responses to generate the group's threshold signature. Note that if any of the checks during signing fails, the (signing) protocol will run from the beginning.
