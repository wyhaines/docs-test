...
ICE-FROST is used in XCP for signing certificates by the sidechain validators. The main purpose of signing is authenticating the sidechain creating the certificate and and verifying the integrity of the certificate i.e., ensuring that the certificate was not altered in transit. It is assumed that sidechains are running a BFT consensus mechanism, so the XCP requires that at least two-thirds (the threshold) of the validators sign the certificate. This ensures that, unless an adversary controls a supermajority of the validators, it is not possible for a sidechain to commit a state which is unknown to honest validators in the certificate.
...
The certificates is signed and then it is propagated with the signature by the sidechain's XCP node. When it reaches the mainchain and/or other XCP nodes, the signature is verified. The signature is indistinguishable from a Schnorr signature. Checking the certificate signature first effectively prevents spamming, in the case an adversary sends multiple certificates with invalid signatures, since it is almost a free operation.
